# This file is a modified version of Julia's MPFR that uses a default precision local 
# to each thread. This allows to use BigFloats safely in a in a multithreaded environment. 
#
# Some functions have been removed --namely conversion of standard floating point types 
# to higher precision -- to avoid collision with the ones in the MPFR package in Base. 
...
function __init__()
    ...
    for i = 1 : length(DEFAULT_PRECISION)
        DEFAULT_PRECISION[i] = Ref{Clong}(256)
    end
...
function get_default_precision()
    tid = Threads.threadid()
    return DEFAULT_PRECISION[tid]
end
...
mutable struct BigFloat <: AbstractFloat
    ...
    function BigFloat(; precision::Integer=get_default_precision()[])
...
function BigFloat(x::BigFloat, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[])
    if precision == MPFR_TLS.precision(x)
        return x
    else
...
for (fJ, fC) in ((:si,:Clong), (:ui,:Culong))
    @eval begin
        function BigFloat(x::($fC), r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[])
            z = BigFloat(;precision=precision)
...
function BigFloat(x::Float64, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[])
    z = BigFloat(;precision=precision)
...
function BigFloat(x::BigInt, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[])
    z = BigFloat(;precision=precision)
    ccall((:mpfr_set_z, :libmpfr), Int32, (Ref{BigFloat}, Ref{BigInt}, MPFRRoundingMode), z, x, r)
    return z
end

BigFloat(x::Integer; precision::Integer=get_default_precision()[]) =
    BigFloat(BigInt(x)::BigInt, ROUNDING_MODE[]; precision=precision)
BigFloat(x::Integer, r::MPFRRoundingMode; precision::Integer=get_default_precision()[]) =
    BigFloat(BigInt(x)::BigInt, r; precision=precision)

BigFloat(x::Union{Bool,Int8,Int16,Int32}, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[]) =
    BigFloat(convert(Clong, x), r; precision=precision)
BigFloat(x::Union{UInt8,UInt16,UInt32}, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[]) =
    BigFloat(convert(Culong, x), r; precision=precision)

BigFloat(x::Union{Float16,Float32}, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[]) =
    BigFloat(Float64(x), r; precision=precision)

function BigFloat(x::Rational, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[])
...
function tryparse(::Type{BigFloat}, s::AbstractString; base::Integer=0, precision::Integer=get_default_precision()[], rounding::MPFRRoundingMode=ROUNDING_MODE[])
...
BigFloat(x::AbstractString, r::MPFRRoundingMode=ROUNDING_MODE[]; precision::Integer=get_default_precision()[]) =
    parse(BigFloat, x; precision=precision, rounding=r)

Rational(x::BigFloat) = convert(Rational{BigInt}, x)

BigFloat(x::Real, r::RoundingMode; precision::Integer=get_default_precision()[]) =
    BigFloat(x, convert(MPFRRoundingMode, r); precision=precision)::BigFloat
BigFloat(x::AbstractString, r::RoundingMode; precision::Integer=get_default_precision()[]) =
...
function big_ln2()
    c = BigFloat()
    ccall((:mpfr_const_log2, :libmpfr), Cint, (Ref{BigFloat}, MPFRRoundingMode), c, MPFR_TLS.ROUNDING_MODE[])
    return c
end
...
"""
precision(::Type{BigFloat}) = Int(get_default_precision()[]) # precision of the type BigFloat itself
